...
def main(args: Optional[list[str]] = None) -> int:
    ...
    analyze_parser.add_argument("image", help="Name/tag of the Docker image (e.g. nginx:latest)")
    analyze_parser.add_argument("--format", choices=["text", "json", "markdown", "html"], default="text", help="Output format")
    analyze_parser.add_argument("--remote", action="store_true", help="Scan image directly from remote registry without pulling")
    ...
    try:
        ...
        if getattr(parsed_args, "remote", False):
            ...
            print(
                ...
                f"Pull it first with `docker pull {e.image_name}` or use the `--remote` flag to scan online.",
                file=sys.stderr
            )
        return 1
