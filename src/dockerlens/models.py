...
class ImageReport:
    ...
    def to_markdown(self) -> str:
        ...
        if not self.audit_results:
            ...
            for r in self.audit_results:
                ...
                lines.append(
                    f"| **{r.severity}** | `{r.rule_id}` | {r.message} | {layer_str} |"
                )
    ...
    def to_html(self) -> str:
        ...
        if not self.audit_results:
            ...
            html.append(
                "<table border='1'><tr><th>Severity</th><th>Rule</th>"
                "<th>Message</th><th>Layer</th></tr>"
            )
            for r in self.audit_results:
                layer_str = str(r.layer_index) if r.layer_index is not None else "-"
                html.append(
                    f"<tr><td><b>{r.severity}</b></td><td><code>{r.rule_id}</code></td><td>{r.message}</td><td>{layer_str}</td></tr>"
                )
            html.append("</table>")

        html.append("<h2>Layers</h2>")
        html.append(
            "<table border='1'><tr><th>Index</th><th>Size</th><th>Command</th></tr>"
        )
        for layer in self.layers:
            ...
            if len(cmd) > 100:
                cmd = cmd[:97] + "..."
            html.append(
                f"<tr><td>{layer.index}</td><td>{layer.size_human}</td><td><code>{cmd}</code></td></tr>"
            )
        html.append("</table>")
